      ...
      <div class="w-100">
        <div class="row">
                  ...
                  <div class="col-4 pl-2">
                    ...
                    <div class="row">
                      ...
                      <div class="col-6">
                        ...
                        @else
                          <span
                            class="badge bg-success">{{ Carbon\Carbon::parse($item->warranty_date)->diffInDays(date('Y-m-d')) }}
                            Hari Lagi</span>
                        @endif
                      </div>
                      <div class="col-6">
                      ...
                      <div class="col-6">
                        ...
                        @else
                          <span
                            class="badge bg-primary">{{ Carbon\Carbon::parse($item->maintenance_date_first)->diffInDays(date('Y-m-d')) }}
                            Hari Lagi</span>
                        @endif
                      </div>
                      <div class="col-6">
                      ...
                      <div class="col-6">
                        ...
                        @else
                          <span
                            class="badge bg-primary">{{ Carbon\Carbon::parse($item->maintenance_date_second)->diffInDays(date('Y-m-d')) }}
                            Hari Lagi</span>
                        @endif
                      </div>
                      <div class="col-6">
                      ...
                      <div class="col-6">
                        ...
                        @else
                          <span
                            class="badge bg-primary">{{ Carbon\Carbon::parse($item->maintenance_date_third)->diffInDays(date('Y-m-d')) }}
                            Hari Lagi</span>
                        @endif
                      </div>
                    </div>
          ...
          <div class="col-12 mt-4">
            <div class="card p-4">
              <div class="card-body">
                <h2 class="mb-4">Detail Laporan Kendala Alat</h2>
                <form method="POST" action="{{ route('admin.report-problems.change',$report->id) }}" class="form mb-4 row">
                    @csrf
                    <div class="form-group col-5">
                        <label for=""><b>Status Laporan</b></label>
                        <select name="status" id="" class="form-control">
                            <option value="0" {{ $report->status == 0?'selected':'' }}>Belum Ditanggapi</option>
                            <option value="1" {{ $report->status == 1?'selected':'' }}>Ditanggapi</option>
                            <option value="2" {{ $report->status == 2?'selected':'' }}>Selesai</option>
                        </select>
                    </div>
                ...
                <div class="row">
                  <div class="col-12">
                    ...
                    <textarea name="" class="form-control" id="" cols="30" rows="10" disabled>{{ $report->description }}</textarea>
                  </div>
                  <div class="col-4 mt-4">
                    <h6>Tanggal Laporan</h6>
                    <input type="text" class="form-control"
                      value="{{ Carbon\Carbon::parse($report->created_at)->format('d M Y') }}" disabled>
                  </div>
                  <div class="col-4 mt-4">
                    <h6>Tanggal Tanggapan</h6>
                    <input type="text" class="form-control"
                      value="{{ $report->response_at?Carbon\Carbon::parse($report->response_at)->format('d M Y'):'' }}" disabled>
                  </div>
                  <div class="col-4 mt-4">
                    <h6>Tanggal Selesai</h6>
                    <input type="text" class="form-control"
                      value="{{ $report->finished_at?Carbon\Carbon::parse($report->finished_at)->format('d M Y'):'' }}" disabled>
                  </div>
                  @foreach ($report->images as $image)

                    <div class="col-4 mt-4">
                      <img src="{{ asset('storage/' . $image->image) }}" style="width: 100%" alt="" class="img-fluid">
                    </div>
                  @endforeach
                </div>
              </div>
